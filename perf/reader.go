...
// Mute stops all notifications from this perf reader.
//
// Subsequent calls to Read will block until a call to Unmute.
func (pr *Reader) Mute() error {
	for i := 0; i < len(pr.rings); i++ {
		if err := pr.array.Delete(uint32(i)); err != nil && errors.Cause(err) != unix.ENOENT {
...
// Unmute allows this perf reader to emit notifications.
//
// Subsequent calls to Read will block until the next event notification.
func (pr *Reader) Unmute() error {
	for i := 0; i < len(pr.rings); i++ {
